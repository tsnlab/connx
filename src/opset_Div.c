...
static bool Div_normal(connx_Backend* backend, connx_DataType type, uint32_t total, void* C, void* A, void* B) {
	switch(type) {
		case connx_UINT32:
			{
...
static bool Div_leaf(connx_Backend* backend, connx_DataType type, uint32_t C_length, void* C, uint32_t A_length, void* A, uint32_t B_length, void* B) {
	switch(type) {
		case connx_UINT32:
			{
				uint32_t* a = (uint32_t*)A;
...
static bool Div_broadcast(connx_Backend* backend, connx_DataType type, uint32_t C_dimension, uint32_t* C_lengths, void* C, uint32_t A_dimension, uint32_t* A_lengths, void* A, uint32_t B_dimension, uint32_t* B_lengths, void* B) {
	if(A_dimension == B_dimension) {
		...
		return Div_normal(backend, type, total, C, A, B);
	}

broadcast:

	if(A_dimension == 1 && B_dimension == 1) {
		return Div_leaf(backend, type, C_lengths[0], C, A_lengths[0], A, B_lengths[0], B);
	}
	...
	if(A_dimension == B_dimension) {
		FOR(C, A, B) {
			bool result = Div_broadcast(backend, type, 
...
bool opset_Div(connx_Backend* backend, uint32_t counts, uint32_t* params) {
	...
	if(connx_Tensor_is_shape_equals(A, B)) {
		return Div_normal(backend, C->type, connx_Tensor_total(C), C->base, A->base, B->base);
	} else {
		return Div_broadcast(backend, C->type, C->dimension, C->lengths, C->base, A->dimension, A->lengths, A->base, B->dimension, B->lengths, B->base);
	}
}
