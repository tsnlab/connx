...
static bool Div_resolve(uintptr_t* stack) {
...
static bool Div_normal(connx_DataType type, uint32_t total, void* C, void* A, void* B) {
	switch(type) {
...
static bool Div_leaf(connx_DataType type, uint32_t C_length, void* C, uint32_t A_length, void* A, uint32_t B_length, void* B) {
	switch(type) {
		case connx_DataType_UINT32:
			{
				uint32_t* a = (uint32_t*)A;
...
static bool Div_broadcast(connx_DataType type, uint32_t C_dimension, uint32_t* C_lengths, void* C, uint32_t A_dimension, uint32_t* A_lengths, void* A, uint32_t B_dimension, uint32_t* B_lengths, void* B) {
	if(A_dimension == B_dimension) {
		...
		return Div_normal(type, total, C, A, B);
	}

broadcast:

	if(A_dimension == 1 && B_dimension == 1) {
		return Div_leaf(type, C_lengths[0], C, A_lengths[0], A, B_lengths[0], B);
	}
	...
	if(A_dimension == B_dimension) {
		FOR(C, A, B) {
			bool result = Div_broadcast(type, 
...
static bool Div_exec(uintptr_t* stack) {
	connx_Tensor* C = (void*)stack[1];
	connx_Tensor* A = (void*)stack[2];
	connx_Tensor* B = (void*)stack[3];

	if(connx_Tensor_isShapeEquals(A, B)) {
		return Div_normal(C->elemType, connx_Tensor_total(C), C->base, A->base, B->base);
	} else {
		return Div_broadcast(C->elemType, C->dimension, C->lengths, C->base, A->dimension, A->lengths, A->base, B->dimension, B->lengths, B->base);
	}
}

bool connx_opset_Div_init() {
	connx_Operator_add("Div", 1, 2, 0, Div_resolve, Div_exec,
