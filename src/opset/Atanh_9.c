...
// clang-format off
int Atanh_{{op_version}}(connx_Graph* graph, __attribute__((unused)) uint32_t output_count, uint32_t* outputs,
                         // clang-format on
    ...
    switch (X->dtype) {
        ...
        /*{% set atanh_func = 'atanhf' if DTYPE == FLOAT32 else 'atanh' %}*/
    case {{ DTYPE }}: {
        {{TYPE}}* X_array = X->buffer;
        {{TYPE}}* Y_array = Y->buffer;

        for (int32_t i = 0; i < total; i++) {
            Y_array[i] = {{atanh_func}}(X_array[i]);
        }
        break;
    }
        /*{% endfor %}*/
    default:
        connx_error("Atanh: Datatype %d is not supported yet.\n", X->dtype);
