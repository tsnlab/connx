...
// clang-format off
int Ceil_{{op_version}}(connx_Graph* graph, __attribute__((unused)) uint32_t output_count, uint32_t* outputs,
                         // clang-format on
    ...
    switch (X->dtype) {
        ...
        /*{% set ceil_func = 'ceilf' if DTYPE == FLOAT32 else 'ceil' %}*/
    case {{ DTYPE }}: {
        {{TYPE}}* X_array = X->buffer;
        {{TYPE}}* Y_array = Y->buffer;

        for (int32_t i = 0; i < total; i++) {
            Y_array[i] = {{ceil_func}}(X_array[i]);
        }
        break;
    }
        /*{% endfor %}*/
    default:
        connx_error("Ceil: Datatype %d is not supported yet.\n", X->dtype);
