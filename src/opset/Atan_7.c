...
// clang-format off
int Atan_{{op_version}}(connx_Graph* graph, __attribute__((unused)) uint32_t output_count, uint32_t* outputs,
                        // clang-format on
    ...
    switch (X->dtype) {
        ...
        /*{% set atan_func = 'atanf' if DTYPE == FLOAT32 else 'atan' %}*/
    case {{ DTYPE }}: {
        {{TYPE}}* X_array = X->buffer;
        {{TYPE}}* Y_array = Y->buffer;

        for (int32_t i = 0; i < total; i++) {
            Y_array[i] = {{atan_func}}(X_array[i]);
        }
        break;
    }
        /*{% endfor %}*/
    default:
        connx_error("Atan: Datatype %d is not supported yet.\n", X->dtype);
