...
int Softplus(connx_Graph* graph, __attribute__((unused)) uint32_t output_count, uint32_t* outputs,
    ...
    switch (X->dtype) {
        ...
        /*{% set ln_func = 'logf' if DTYPE == FLOAT32 else 'logl' %}*/
    case {{ DTYPE }}: {
        {{TYPE}}* X_array = X->buffer;
        {{TYPE}}* Y_array = Y->buffer;

        for (int32_t i = 0; i < total; i++) {
            Y_array[i] = {{ln_func}}({{exp_func}}(X_array[i]) + 1);
        }
        break;
    }
        /*{% endfor %}*/
    default:
        connx_error("SoftPlus: Datatype %d is not supported yet.\n", X->dtype);
