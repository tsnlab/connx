...
// clang-format off
int Elu_{{op_version}}(connx_Graph* graph, __attribute__((unused)) uint32_t output_count, uint32_t* outputs,
                        // clang-format on
    ...
    case {{ DTYPE }}: {
        ...
        for (int32_t i = 0; i < total; i++) {
            Y_array[i] = (alpha * (exp(X_array[i]) - 1) * (X_array[i] < 0)) + (X_array[i] * (X_array[i] >= 0));
        }
        break;
    }
        /*{% endfor %}*/
