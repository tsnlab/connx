...
// clang-format off
int Asinh_{{op_version}}(connx_Graph* graph, __attribute__((unused)) uint32_t output_count, uint32_t* outputs,
                          // clang-format on
    ...
    switch (X->dtype) {
        ...
        /*{% set asinh_func = 'asinhf' if DTYPE == FLOAT32 else 'asinh' %}*/
    case {{ DTYPE }}: {
        {{TYPE}}* X_array = X->buffer;
        {{TYPE}}* Y_array = Y->buffer;

        for (int32_t i = 0; i < total; i++) {
            Y_array[i] = {{asinh_func}}(X_array[i]);
        }
        break;
    }
        /*{% endfor %}*/
    default:
        connx_error("Asinh: Datatype %d is not supported yet.\n", X->dtype);
