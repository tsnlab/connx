...
def postprocess_boxes(pred_bbox, org_img_shape, input_size, score_threshold):
    ...
    ratio_w = org_w / input_size
    ratio_h = org_h / input_size

    # dw = (input_size - resize_ratio * org_w) / 2
    # dh = (input_size - resize_ratio * org_h) / 2

    # print(f'{dw=}, {dh=}, {resize_ratio=}')
...
def load_image(img_path: str) -> np.ndarray:
    ...
    # Convert RGB to BGR
    img = img.convert('RGB')
    red, green, blue = np.array(img).T
    bgr_data = np.array([blue, green, red]).transpose()
    img = Image.fromarray(bgr_data)
...
if __name__ == '__main__':
    ...
    bboxes = postprocess_boxes(pred_bbox, img_orig.size, input_size, 0.25)
    bboxes = nms(bboxes, 0.213, method='nms')
    image = draw_bbox(img_orig, bboxes)
